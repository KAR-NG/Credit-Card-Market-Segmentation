---
title: "Credit Card Market Segmentation"
author: "Kar Ng"
date: '2022-05'
output: 
  github_document: 
...
```
Plotting Radar plot to see how distinctive each of the 6 cluster is to each other. 
...
Two clusters have been identified from the entire dataset and are summarised in following manner: 
...
ggparcoord(fuz_df, 
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = "Parallel Coordinate Plot",
       subtitle = "Showing of All coordinates in each variables")


```

Fuzzy clustering with k equal to 7 have characterise the 7 clusters in following ways:

**Cluster 1**: General users without distintive features.

**Cluster 2**: Revolvers. This group of users are characterised by having the highest balance of credit in their credit card, they do not prefer to pay full payment and prefer to pay minimum payments. They use credit card to make various purcharses but particularly loving to use cash-in-advance.

**Cluster 3**: Active purchasers, they generally love to use credit card for various purchases, and have a high credit limit, they are in particular the group who do not like to use cash-in-advance.

**Cluster 4**: One-off buyers, this group of card users love to use credit card for one-off payment and pay back with a tenure term.

**Cluster 5**: Non-Payers, they have balance in their account, but have the lowest level of balance frequency, which is the frequency of balance update when purchases are made or when payments are paid.

**Cluster 6**: Zero tenure users, this group is characterised by zero tenure term.

**Cluster 7**: Max payer, this group of users prefer to pay full payment back to credit card company and this explains why this group of users have the least balance in their credit card.
...
The gemetric features (volume, shape, and orientation) of each cluster will then be determined. After this step, the similarity of volume, shape and orientation between clusters of that k will be determine using a standard parameterasation.
...
**Cluster 1** : Revolvers. They have the lowest level of tenure and full repayment rate, they owe credit card company a lot of money as indicated by the highest level of balance, and very low balance frequency which can be an indicator of how debt are being paid. This group of users use card for many purchases but most preferably using cash-in-advance, and therefore they are given the highest credit limit.

**Cluster 2** : Cash-user. This group of users prefer to use cash in advance and paying back but not full in amount. They have high tenure term, balance and balance frequency in their card.

**Cluster 3** : Max payer. Users from this group have the highest level of full payment and therefore the lowest level of balance and balance frequency. They have the lowest credit limit and relatively less active use of the company's credit card.

**Cluster 4** : Purchaser. Compared to cluster 2, this user group prefer to use credit card actively for various purchases. They do not like to use cash in advance, and they like to pay full payment back to credit card company. This explains why they have lower level of balance in their card that needed to pay back the credit card company.

## 7 CONCLUSION

Many of the famous algorithm have been performed in this project, include:

-   CLARA from partitioning clustering, which have identified 2 distinctive group of users with its optimal number of K suggested by silhouette plot based on CLARA clustering method. Two identified groups are "Cash-in-advance users" and "Max-use Max-pay users".

-   Hierarchical K-mean Clustering, which is considered an advanced type of clustering combining two algorithm sub-category of partitioning clustering. This algorithm identified 9 groups of users as suggested by silhouette plot based on k-mean clustering method. These 9 groups of users are "Least active users", "Cash users", "One-off users", "Average card users", "Installment users", "Inactive users", "Zero-tenure group", "Active users", and "Minimum payment users".

-   Fuzzy c-mean clustering, which is a soft clustering technique that allow an observation to be able to belong to multiple clusters by given it probability value. This algorithm identified 7 groups of users which are "General users", "Revolvers", "Active purchasers", "One-off buyers", "Non-Payers", "Zero tenure users", and "Max payer".

-   Model-based clustering identified the most distinctive 4 groups of credit card users which are revolvers, cash-user, max payer, and purchaser.
